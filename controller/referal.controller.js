...
const acceptPreReferral = async (req, res) => {
  try {
      ...
      if (!referralId) {
          return res.status(400).json({
              message: "Missing required fields: referralId or therapistId.",
              error: true,
          });
      }
      ...
      if (!ObjectId.isValid(referralId)) {
          return res.status(400).json({
              message: "Invalid referralId or therapistId.",
              error: true,
          });
      }
...
const getReceivedPreconsultationReferralList = async (req, res) => {
  try {
    ...
    // Fetch all referrals where the logged-in therapist is in the therapists array
    ...
    clients.forEach(client => {
      clientMap[client._id.toString()] = {
        clientName: client.name,
        phoneNumber: client.phone_number,
        clientHistory: client.history || "No history available", // Assuming history is stored in the client object
      };
    });

    // Format the response with referralStatus, noteStatus, and client details
    const response = receivedReferrals.map(referral => {
      const therapistData = referral.therapists.find(therapist =>
        therapist.therapist_id.equals(therapist._id)
      );
      ...
      return {
        ...
        referralId: referral._id,
        referralStatus: therapistData?.status,
        createdAt: referral.created_at,
        notes: referral.notes,
        type : referral.type
      };
...
const getReceivedSessionReferralList = async (req, res) => {
  try {
    ...
    console.log("refrals>>",receivedReferrals);
    ...
    const response = await Promise.all(receivedReferrals.map(async (referral) => {
      ...
      //   therapist => therapist.therapist_id.equals(new ObjectId(therapist._id))
      // );
      const therapistReferralStatus = referral.therapists.find(
...
const getListOfTherapistSuggested = async (req, res) => {
  ...
  try {
    ...
    // Fetch all referrals where client_id matches the user ID
    const referrals = await referralsCollection.find({ client_id: user._id }).toArray();

    // Extract unique therapist IDs from the referrals
    const therapistIds = [];
    referrals.forEach(referral => {
      referral.therapists.forEach(therapist => {
        if (!therapistIds.includes(therapist.therapist_id)) {
    ...
    if (therapistIds.length === 0) {
      return res.status(404).json({
        message: "No therapists found for the user",
        error: true,
      });
    }
